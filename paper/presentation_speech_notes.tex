...
\paragraph{时变条件方差（Time-Varying Conditional Variance）} 条件方差是在给定过去信息（即历史收益率）的前提下，对下一期收益率方差的估计。符号记为 \(\sigma_t^2 = \text{Var}(r_t \mid \mathcal{F}_{t-1})\)，其中 \(\mathcal{F}_{t-1}\) 表示到 \(t-1\) 时刻为止的所有历史信息。
...
\subsection*{讲稿}
这是本文的主模型——GARCH(1,1)。首先看条件方差的定义：\(\sigma_t^2 = \text{Var}(r_t \mid \mathcal{F}_{t-1})\)。意思是：在已知截至昨天所有历史信息的条件下，今天收益率的不确定性有多大。条件方差随时间变化——市场动荡时高，平静时低。

模型由两个方程组成。均值方程很简单：\(r_t = \mu + \varepsilon_t\)。就是说收益率等于一个常数加上一个随机扰动。我们不试图预测方向。
...
\begin{align*}
\sigma_t^2 &= \text{Var}(r_t \mid \mathcal{F}_{t-1}) \quad \text{（条件方差定义）}\\
...
\end{align*}
其中 \(\mathcal{F}_{t-1}\) 表示到 \(t-1\) 时刻为止的所有历史信息（即过去所有收益率的集合）。
...
模型结构和GARCH(1,1)完全一样——同样的均值方程和方差方程——唯一的区别是：把 \(z_t \stackrel{i.i.d.}{\sim} N(0,1)\) 换成 \(z_t \stackrel{i.i.d.}{\sim} t(\nu)\)。这里\(\nu\)是自由度参数，控制尾部的厚薄程度。PPT上给出了标准化Student-t分布的密度函数公式，分母里\(\sqrt{\nu-2}\)保证了方差为1。

结果非常明确。估计的自由度约5.15——自由度越小尾部越厚，5.15这个值确认了收益率确实比正态分布厚尾很多。而且 \(\nu \to \infty\) 时t分布趋近正态，所以5.15这个值离无穷远很远，厚度差异很大。
...
\paragraph{Student-t GARCH(1,1) 模型设定}
\begin{align*}
r_t &= \mu + \varepsilon_t, \quad \varepsilon_t = \sigma_t z_t\\
\sigma_t^2 &= \omega + \alpha \varepsilon_{t-1}^2 + \beta \sigma_{t-1}^2\\
z_t &\stackrel{i.i.d.}{\sim} t(\nu), \quad \nu > 2
\end{align*}
标准化Student-t密度函数（方差=1）：
\[
f(z;\nu) = \frac{\Gamma(\frac{\nu+1}{2})}{\sqrt{(\nu-2)\pi}\,\Gamma(\frac{\nu}{2})}
\left(1 + \frac{z^2}{\nu-2}\right)^{-(\nu+1)/2}
\]
其中 \(\Gamma(\cdot)\) 是Gamma函数。分母中的 \(\sqrt{\nu-2}\) 使标准化后的分布方差为1，与标准正态 \(N(0,1)\) 的方差一致。约束 \(\nu > 2\) 保证方差存在。
...
第五也是最重要的一点：非对称效应有部分证据，但对模型形式敏感。GJR-Student-t下非对称项显著，但EGARCH-t下不显著。
...
第一，GARCH是reduced-form模型。它描述的是条件方差\"如何\"随时间演化，而不是\"为什么\"产生。

第二，中国股市在样本期内经历了多次制度变革——股权分置改革、融资融券、熔断机制等，但本模型没有显式处理这些结构突变。

第三，模型只用了收益率自身的历史信息，没有纳入货币政策、流动性、国际市场联动等外部因素。

第四，关于非对称效应：GJR-t下显著，但EGARCH证据较弱
