...
\begin{frame}{稳健性2: GJR-GARCH(1,1)}
  ...
  \end{block}
  \begin{itemize}
    \item $\hat{\gamma}=0.0159$，$p=0.217$ \quad $\Rightarrow$ \textbf{为正但不显著}
    \item 不能强断言存在杠杆效应
    \item AIC仅略优于对称GARCH（20312 vs 20314），BIC更高
  \end{itemize}
  \begin{block}{解释}
    在本样本区间，沪深300指数层面的非对称波动效应\textbf{强度有限}，\\
    或可能被正向波动事件所稀释。
  \end{block}
\end{frame}

% === Slide 16: 稳健性 — AR(1)-GARCH ===
\begin{frame}{稳健性3: AR(1)-GARCH(1,1)}
  \begin{block}{均值方程}
...
\begin{frame}{局限与证据边界}
  \begin{itemize}
    ...
    \item \textbf{GJR-GARCH $\gamma$不显著}\\
    在本文样本下不能强断言杠杆效应
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \vspace{2em}
  \begin{flushright}
    \small 代码与数据：\texttt{github.com/ljq2088/hs300-garch-volatility}
  \end{flushright}
\end{frame}
